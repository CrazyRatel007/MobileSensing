...
int main(int argc, char* argv[])
{
	...
	for (int i = 1; i <= 10; i++)
		tTaxiRandomVisitRoadProbability[i] = ReadTaxiVisitedProbability(tTotalEdge,std::to_string(i) + "Taxi-Probability.csv");

	std::vector<ONEINTONEFLOAT>* pDeterministicVisitRoadProbability = new std::vector<ONEINTONEFLOAT>[9];
	for (int i = 1; i <= 9; i++)
		pDeterministicVisitRoadProbability[i] = ReadBusVisitedRoad(tTotalEdge,"Bus-" + std::to_string(i));

	std::vector<float> tRoadLengthSet = ReadRoadLength(tTotalEdge,"EdgeLength.csv.csv");
